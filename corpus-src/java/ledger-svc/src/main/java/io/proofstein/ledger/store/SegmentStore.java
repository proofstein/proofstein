...
public final class SegmentStore {
    ...
    public static final class Sealed implements Store {
        ...
        public void put(String key, byte[] value) throws GeneralSecurityException {
            items.put(key, envelope.seal(value, key.getBytes(StandardCharsets.UTF_8))); //@PS java-l3-wrapper|AES-256-GCM|3|algorithm|AEAD reached only through the store registry
        }
    ...
    public static Store open(String name, byte[] dataKey) {
        ...
        return factory.apply(dataKey); //@PS +java-l3-wrapper
    }
}
