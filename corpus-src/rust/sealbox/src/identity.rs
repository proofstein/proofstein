...
use ed25519_dalek::{Signer, SigningKey}; //@PS +rust-l1-ed25519
...
impl Identity {
    ...
    pub fn generate() -> Result<Self, rsa::Error> {
        let manifest = SigningKey::generate(&mut OsRng); //@PS rust-l1-ed25519|Ed25519|1|algorithm|manifest signing key
        let peer_tls = EcdsaSigningKey::random(&mut OsRng); //@PS rust-l1-ecdsa|ECDSA-P256|1|algorithm|peer mutual TLS key
        let archive_gateway = RsaPrivateKey::new(&mut rand::thread_rng(), ARCHIVE_KEY_BITS)?; //@PS rust-l1-rsa|RSA-2048|1|algorithm|archive gateway key
        Ok(Self {
            manifest,
            peer_tls,
            archive_gateway,
    ...
    pub fn peer_thumbprint(&self) -> Vec<u8> {
        ...
        Sha256::digest(encoded).to_vec() //@PS rust-l1-sha256|SHA-256|1|algorithm|peer key thumbprint
    }
