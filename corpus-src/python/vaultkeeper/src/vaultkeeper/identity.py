...
class NodeIdentity:
    ...
    def generate(cls) -> "NodeIdentity":
        audit = rsa.generate_private_key(public_exponent=65537, key_size=AUDIT_KEY_BITS)  #@PS py-l1-rsa|RSA-2048|1|algorithm|legacy audit export key
        peer = ec.generate_private_key(ec.SECP256R1())  #@PS py-l1-ecdsa|ECDSA-P256|1|algorithm|peer mesh TLS key
        manifest = ed25519.Ed25519PrivateKey.generate()  #@PS py-l1-ed25519|Ed25519|1|algorithm|manifest signing key
    ...
    def sign_audit_export(self, export: bytes) -> bytes:
        ...
        return self.audit_export.sign(
            ...
            hashes.SHA256(),  #@PS py-l1-sha256|SHA-256|1|algorithm|digest algorithm for audit export signatures
        )
