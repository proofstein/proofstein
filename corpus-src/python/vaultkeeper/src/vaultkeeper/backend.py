...
class SealedStore:
    ...
    def put(self, key: str, blob: bytes) -> None:
        sealed = envelope.seal(self._data_key, blob, key.encode())  #@PS py-l3-wrapper|AES-256-GCM|3|algorithm|AEAD reached only through the backend registry
...
def open_backend(name: str, data_key: bytes) -> Backend:
    ...
    return factory(data_key)  #@PS +py-l3-wrapper
